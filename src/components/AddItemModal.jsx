        ...
        {type === "vinyl" ? (
            ...
            {vinylMode === "search" ? (
              <>
                <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                  <input value={vinylQuery} onChange={(e) => setVinylQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && searchVinyl()} placeholder="Artist or album name..." style={{ flex: 1, padding: "12px 14px", borderRadius: 12, border: "1px solid rgba(13,148,136,0.25)", background: "rgba(255,255,255,0.7)", fontFamily: F, fontSize: 14, color: "#1e1b4b", outline: "none" }} />
                  <button onClick={searchVinyl} disabled={vinylSearching} style={{ padding: "12px 20px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #0d9488, #10b981)", color: "#fff", fontFamily: F, fontSize: 14, fontWeight: 600, cursor: vinylSearching ? "wait" : "pointer" }}>🔍</button>
                </div>
                {vinylSearchError && <div style={{ fontFamily: F, fontSize: 13, color: "#dc2626", marginBottom: 12 }}>{vinylSearchError}</div>}
                {vinylSearching && <div style={{ textAlign: "center", padding: 24, fontFamily: F, fontSize: 13, color: "#0f766e" }}>Searching Discogs…</div>}
                {!vinylSearching && vinylResults.length > 0 && (
                  <div style={{ maxHeight: 280, overflowY: "auto", marginBottom: 16, border: "1px solid rgba(13,148,136,0.15)", borderRadius: 12 }}>
              ...
              <>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                  <input type="checkbox" checked={isLimited} onChange={() => setIsLimited(!isLimited)} style={{ accentColor: "#0d9488" }} />
          ...
          <>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{types.map((t) => (<button key={t} onClick={() => setMerchType(t)} style={{ padding: "7px 14px", borderRadius: 20, border: `1px solid ${merchType === t ? "#0d9488" : "rgba(13,148,136,0.2)"}`, background: merchType === t ? "rgba(13,148,136,0.12)" : "transparent", fontFamily: F, fontSize: 12, fontWeight: merchType === t ? 700 : 500, color: merchType === t ? "#0f766e" : "rgba(55,48,107,0.5)", cursor: "pointer" }}>{t}</button>))}</div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <input type="checkbox" checked={isTour} onChange={() => setIsTour(!isTour)} style={{ accentColor: "#0d9488" }} />
