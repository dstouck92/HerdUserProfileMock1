        ...
        {mode === "search" ? (
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && search()} placeholder="Search artist name..." style={{ flex: 1, padding: "12px 14px", borderRadius: 12, border: "1px solid rgba(13,148,136,0.25)", background: "rgba(255,255,255,0.7)", fontFamily: F, fontSize: 14, color: "#1e1b4b", outline: "none" }} />
            ...
            {!searching && searchResults.length > 0 && (
              <>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontFamily: F, fontSize: 13, fontWeight: 600, color: "#0f766e", display: "block", marginBottom: 6 }}>Concert details — name, venue, etc.</label>
                  <input value={concertDetailsFilter} onChange={(e) => setConcertDetailsFilter(e.target.value)} placeholder="Filter by venue, tour, city…" style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid rgba(13,148,136,0.25)", background: "rgba(255,255,255,0.7)", fontFamily: F, fontSize: 14, color: "#1e1b4b", outline: "none", boxSizing: "border-box" }} />
                </div>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontFamily: F, fontSize: 13, fontWeight: 600, color: "#0f766e", display: "block", marginBottom: 6 }}>Year</label>
                  <input type="text" inputMode="numeric" value={yearFilter} onChange={(e) => setYearFilter(e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="e.g. 2024" style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid rgba(13,148,136,0.25)", background: "rgba(255,255,255,0.7)", fontFamily: F, fontSize: 14, color: "#1e1b4b", outline: "none", boxSizing: "border-box" }} />
                </div>
                ...
                <div style={{ maxHeight: 260, overflowY: "auto" }}>
                  {filteredResults.length === 0 ? <div style={{ padding: 20, textAlign: "center", fontFamily: F, fontSize: 13, color: "rgba(55,48,107,0.5)" }}>No concerts match the filters.</div> : filteredResults.map((c, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", marginBottom: 8, background: "rgba(13,148,136,0.08)", borderRadius: 12, border: "1px solid rgba(13,148,136,0.12)" }}>
                      ...
                      <button onClick={() => addFromSearch(c)} style={{ marginLeft: 12, padding: "8px 16px", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #0d9488, #10b981)", color: "#fff", fontFamily: F, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Add</button>
                    </div>
                  ))}
                </div>
              </>
