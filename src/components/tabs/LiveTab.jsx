  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 20px", marginBottom: 10 }}>
        ...
        <button onClick={onAdd} style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid rgba(13,148,136,0.25)", background: "rgba(255,255,255,0.6)", fontFamily: F, fontSize: 12, fontWeight: 600, color: "#0d9488", cursor: "pointer" }}>+ Add</button>
      </div>
      <Card>
        {concerts.map((c, i) => (
          <div key={c.id} style={{ display: "flex", alignItems: "center", padding: "14px 20px", borderBottom: i === concerts.length - 1 ? "none" : "1px solid rgba(13,148,136,0.1)" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: F, fontSize: 12, color: "rgba(55,48,107,0.5)", marginTop: 2 }}><span style={{ fontWeight: 600, color: "#0d9488" }}>{c.date}</span>{c.venue && <> | {c.venue}</>}{c.city && <>, {c.city}</>}</div>
            </div>
            <button onClick={() => onEdit(c)} type="button" title="Edit concert" style={{ marginRight: 8, padding: "8px 10px", borderRadius: 10, border: "1px solid rgba(13,148,136,0.25)", background: "rgba(255,255,255,0.6)", color: "#0d9488", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} aria-label="Edit">✎</button>
            <span style={{ color: "rgba(13,148,136,0.4)", fontSize: 18 }}>›</span>
          </div>
        ))}
      </Card>
    </div>
