  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 20px", marginBottom: 4 }}>
        ...
        <button onClick={onAddVinyl} style={{ padding: "5px 12px", borderRadius: 16, border: "1px solid rgba(99,102,241,0.2)", background: "rgba(255,255,255,0.6)", fontFamily: F, fontSize: 11, fontWeight: 600, color: "#6366f1", cursor: "pointer" }}>+ Add</button>
      </div>
      {vinyl.length ? (
        <>
          <div style={{ padding: "0 20px", marginBottom: 10, fontFamily: F, fontSize: 13, color: "rgba(55,48,107,0.5)" }}><strong style={{ color: "#4f46e5" }}>{vinyl.length}</strong> Records | <strong style={{ color: "#4f46e5" }}>{vinyl.filter((v) => v.is_limited_edition).length}</strong> Limited</div>
          <Card>{vinyl.map((v, i) => (
            <div key={v.id} style={{ display: "flex", alignItems: "center", padding: "12px 20px", gap: 14, borderBottom: i < vinyl.length - 1 ? "1px solid rgba(99,102,241,0.06)" : "none" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: F, fontSize: 12, color: "rgba(55,48,107,0.5)", marginTop: 2 }}>{v.album_name}{v.is_limited_edition && <span style={{ color: "#6366f1", fontWeight: 600 }}> · Limited</span>}</div>
              </div>
            </div>
          ))}</Card>
        </>
      ...
      <div style={{ marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 20px", marginBottom: 10 }}>
        ...
        <button onClick={onAddMerch} style={{ padding: "5px 12px", borderRadius: 16, border: "1px solid rgba(99,102,241,0.2)", background: "rgba(255,255,255,0.6)", fontFamily: F, fontSize: 11, fontWeight: 600, color: "#6366f1", cursor: "pointer" }}>+ Add</button>
      </div>
      {merch.length ? (
        <Card>{Object.entries(merchByArtist).map(([a, items], ai) => (
          <div key={a} style={{ padding: "12px 20px", borderBottom: ai < Object.keys(merchByArtist).length - 1 ? "1px solid rgba(99,102,241,0.06)" : "none" }}>
            ...
            {items.map((item) => (
              <div key={item.id} style={{ display: "flex", justifyContent: "space-between", fontFamily: F, fontSize: 13, color: "rgba(55,48,107,0.6)", padding: "3px 0 3px 12px" }}>
                ...
                {item.purchase_price && <span style={{ color: "#4f46e5", fontWeight: 600 }}>${item.purchase_price}</span>}
              </div>
            ))}
          </div>
        ))}</Card>
